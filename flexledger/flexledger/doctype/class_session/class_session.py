...
#    def validate(self):
#     # group E1 - session date
#     if self.docstatus == 0 or self.is_new():
#         if getdate(self.session_date) < getdate():
#             frappe.throw("Session date cannot be in past for Draft record")

#     session_type = frappe.get_doc("Session Type", self.session_type)

#     for attendee in self.attendees:
#         packages = frappe.get_doc("Package Purchase",attendee.package_purchase)
...
#             frappe.throw("Status must be completed before submitting!")
...
class ClassSession(Document):
    ...
    def on_submit(self):
        ...
        for attendee in self.attendees:
            ...
            # Mark package Fully Used when balance = zero
            new_status = (
                "Fully Used"
                if new_credits_remaining == 0
                else package.status
            ...
            # Queue email without blocking session submission
            # if new_credits_remaining <= threshold:
            #     frappe.enqueue(
            #         "flexledger.api.send_low_balance_email",
            #         member=package.member,
